...
public sealed record DoctorProfileListItem(Guid Id, string FullName, string? Specialization, Guid UserId, string? Email, string? PhoneNumber);
...
public sealed class ListDoctorProfilesHandler : IRequestHandler<ListDoctorProfilesQuery, IReadOnlyList<DoctorProfileListItem>>
{
	...
	public async Task<IReadOnlyList<DoctorProfileListItem>> Handle(ListDoctorProfilesQuery request, CancellationToken cancellationToken)
	{
		...
		var list = await _db.DoctorProfiles
			...
			.Select(d => new { d.Id, d.FullName, d.Specialization, d.UserId, d.Email, d.ContactNumber })
		...
		var items = list.Select(d => new DoctorProfileListItem(
			...
			d.Specialization,
			d.UserId,
			_enc.Decrypt(d.Email) ?? d.Email,
			_enc.Decrypt(d.ContactNumber) ?? d.ContactNumber
		));
		if (!string.IsNullOrWhiteSpace(request.Query))
		{
			...
			items = items.Where(x =>
				...
				(x.Specialization ?? string.Empty).ToLowerInvariant().Contains(q) ||
				(x.Email ?? string.Empty).ToLowerInvariant().Contains(q) ||
				(x.PhoneNumber ?? string.Empty).ToLowerInvariant().Contains(q)
			);
		}
		return items.ToList();
	}
